    ...
    public function addSortie(EntityManagerInterface $em, Request $request, EtatRepository $etat, ValidatorInterface $validator): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            if ($dateLimiteInscription != null) {
                if ($dateLimiteInscription > $dateNow) {
                    if ($dateLimiteInscription < $dateDebut) {
                        $sortie->setDateDebut(new \DateTime($dateLimiteInscription));
                    } else
                        $msg_error = "La date de limite d'inscription doit être strictement inférieur à la date de début de la sortie";
                } else
                    $msg_error = "La date doit être supérieur ou égale à la date d'aujourd'hui";
            } else
                $msg_error = "Veuillez saisir une date de limite d'inscription";
